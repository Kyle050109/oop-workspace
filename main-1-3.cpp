...
int main(){
    ...
    int parkingDuration=3600;
    for (int i=0;i<3;i++){
        bool parked=lot.parkVehicle(vehicles[i]);
        if (!parked){
            std::cerr << "Failed to park vehicle with ID " << vehicles[i]->getID() << std::endl;
        }
        vehicles[i]->getParkingDuration(parkingDuration);
    }
    int maxParkingDuration=1800;
    std::cout << "Number of overstaying vehicles: " 
              << lot.countOverstayingVehicles(maxParkingDuration) << std::endl;
    return 0;
    for (int i=0;i<3;i++){
